...
int main(int argc, char *argv[]) {
  ...
  if (!init()) {
    ...
    while (quit == false) {
      ...
      if (vc.wasPressed(Input::LIGHT_P)) {
      }
      ...
      // If the frame finished early, delay to cap at 60 FPS (approx 16ms per
      // frame)
      if (frameTime < 16) {
...
bool init() {
  ...
  if (!SDL_Init(SDL_INIT_VIDEO | SDL_INIT_GAMEPAD)) {
    ...
    if (gWindow = SDL_CreateWindow("MyWindow", cScreenWidth, cScreenHeight, 0);
        gWindow == nullptr) {
...
void openGamePad(SDL_JoystickID which) {
  if (gamePad ==
      nullptr) { /* we don't have a stick yet and one was added, open it! */
    gamePad = SDL_OpenGamepad(which);
    if (!gamePad) {
      SDL_Log("Failed to open joystick ID %u: %s", (unsigned int)which,
              SDL_GetError());
    }
  }
}
